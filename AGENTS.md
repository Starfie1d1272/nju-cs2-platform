...
当前阶段：**v1.0.0 发布整理中。生产站点已部署到 `match.starfie1d.top`，Supabase RLS/Realtime 白名单已配置，Phase 12 仅剩关键路径 E2E 与性能基准作为上线后持续验收项。v2 赛制引擎（StageExecutor v2 + 5 个 executor + entrySeeds 种子轮空 + finalFormat 决赛 BO5）已在 dev 落地。**
...
**禁止手动修改 `package.json` 中的 version 字段。** 必须使用 `npm version <patch|minor|major|x.y.z> --no-git-tag-version`，发布 PR 合并后再在 `main` 的 merge commit 上打 tag。
